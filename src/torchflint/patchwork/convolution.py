...
def masked_conv_transpose_patches(
    ...
    *,
    ...
    Args:
        input (Tensor): [B, C_in, L_0, L_1, ..., L_n]
        weight (Tensor): [B, C_in, C_out // groups, k_L_0, k_L_1, ..., k_L_n]
...
def _conv_transpose_template(
    ...
    *,
    ...
    folder: Callable[[Tensor, Union[int, Sequence[int]], Union[int, Sequence[int]], Union[int, Sequence[int]]]],
    **kwargs
) -> Tensor:
    ...
    output = folder(patches, stride, padding, dilation)
    
    while output_padding != 0:
        if isinstance(output_padding, int):
            output_padding = [pad_value for _ in range(spatial_ndim) for pad_value in (0, output_padding)]
        elif any(value != 0 for value in padding):
            output_padding = [pad_value for value in reversed(output_padding) for pad_value in (0, value)]
        else:
            break
        output = functional_pad(output, output_padding)
        break
