...
def convert_to_floating_image(image: Tensor):
...
def sobel_edges(image: Tensor):
    """Returns a tensor holding Sobel edge maps.
    
    Args:
        image (Tensor): Image tensor with shape [batch_size, channels, height, width], expected a floating point type.
    
    Returns:
        out (Tensor): Tensor holding edge maps for each channel. Returns a tensor with shape
...
def shift_image(image: Tensor, shift: Union[int, Sequence[int]], fill_value = 0) -> Tensor:
